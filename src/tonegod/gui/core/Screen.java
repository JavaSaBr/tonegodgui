...
public class Screen implements ElementManager, Control, RawInputListener {
    ...
    protected static final Logger LOGGER = LoggerManager.getLogger(Screen.class);

    public static final String DEFAULT_STYLE_MAP = "tonegod/gui/style/def/style_map.gui.xml";
    ...
    private static synchronized void checkLoader(final Application application) {
        if (initializedLoader) return;

        final AssetManager assetManager = application.getAssetManager();
        assetManager.registerLoader(StyleLoader.class, "gui.xml");
    ...
    private final ObjectDictionary<String, Element> idToElement;
    ...
    private float zOrderCurrent = 1F;
    private float zOrderStepMajor = 0.1F;
    ...
    /**
     * Creates a new instance of the Screen control using the default style information provided
     * with the library.
     *
     * @param application A JME Application
     */
    public Screen(final Application application) {
        this(application, DEFAULT_STYLE_MAP);
    }
     ...
     */
    public Screen(final Application application, final String styleMap) {
        checkLoader(application);

        final AssetManager assetManager = application.getAssetManager();

        this.idToElement = DictionaryFactory.newObjectDictionary();
        this.elements = ArrayFactory.newArray(Element.class);
        this.application = application;
        this.elementZOrderRay.setDirection(Vector3f.UNIT_Z);
        this.results = new CollisionResults();
        this.styleManager = new StyleManager(this, styleMap);
        this.styleManager.parseStyles(styleMap);
        this.effectManager = new EffectManager(this);
        this.animManager = new AnimManager(this);
        this.layoutParser = new LayoutParser(this);
        this.defaultGUIFont = assetManager.loadFont(styleManager.getStyle("Font").getString("defaultFont"));

        final ViewPort viewPort = application.getViewPort();
        final SafeArrayList<Spatial> scenes = viewPort.getScenes();

        this.scenes.add((Node) scenes.get(0));
        this.scaleManager = new ScaleUtil(this);
        this.scaleManager.initialize();

    }

    public Screen(final Application application, final String styleMap, final float width, final float height) {
        checkLoader(application);
        ...
        this.orDim = true;
        this.inputScale = 1f / (camera.getWidth() / width);
        this.application = application;
        this.elementZOrderRay.setDirection(Vector3f.UNIT_Z);
        this.results = new CollisionResults();
        this.styleManager = new StyleManager(this, styleMap);
    ...
    public float getWidth() {
        return (orDim) ? orWidth : application.getViewPort().getCamera().getWidth();
    }
    ...
    public float getHeight() {
        return (orDim) ? orHeight : application.getViewPort().getCamera().getHeight();
    }
    ...
    public void addElement(final Element element) {
        ...
        if (element instanceof AutoHide) {
            ...
            return;
        } else if (getElementById(element.getUID()) != null) {
            throw new RuntimeException("The child element '" + element.getUID() + "' (" + element.getClass() + ") conflicts with a previously added child element in parent Screen.");
        }

        idToElement.put(element.getUID(), element);
    ...
    public void addElement(Element element, boolean hide) {
        if (element instanceof AutoHide) hide = true;
    ...
    protected void initializeElement(final Element element) {
        if (element.getInitialized()) return;

        element.setY(getHeight() - element.getHeight() - element.getY());

        final Vector2f orgPosition = element.getOrgPosition();
        orgPosition.set(element.getPosition());
        orgPosition.setY(element.getY());
    ...
    public void removeElement(final Element element) {
        idToElement.remove(element.getUID());
        elements.slowRemove(element);

        final float shiftZ = element.getLocalTranslation().getZ();

        for (final Element child : elements.array()) {
            if(child == null) break;
            if (child instanceof ToolTip) continue;

            final Vector3f childLoc = child.getLocalTranslation();
            if (childLoc.getZ() <= shiftZ) continue;

            child.move(0, 0, -zOrderStepMajor);
        }
        ...
        if (!getUseToolTips()) return;

        final Element toolTipFocus = getToolTipFocus();

        if (toolTipFocus == element) hideToolTip();
        else if (toolTipFocus != null) {
            if (element.getChildElementById(toolTipFocus.getUID()) != null) hideToolTip();
        }
    }
     ...
     */
    @Override
    public Element getElementById(final String UID) {

        Element result = idToElement.get(UID);
        if(result != null) return result;

        for (final Element child : elements.array()) {
            if (child == null) break;
            result = child.getChildElementById(UID);
    ...
    /**
     * Returns the screen level idToElement Map
     */
    public ObjectDictionary<String, Element> getElementsAsMap() {
        return idToElement;
    }

    /**
     * Returns the screen level idToElement as a Collection
     */
    ...
    public void setUseTextureAtlas(boolean useTextureAtlas, String texturePath) {
        ...
        if (texturePath != null) {
            atlasTexture = application.getAssetManager().loadTexture(texturePath);
    ...
    public Texture createNewTexture(String texturePath) {
        Texture newTex = application.getAssetManager().loadTexture(texturePath);
    ...
    private void throwParserException() {
        try {
           ...
           LOGGER.error("The provided texture information does not conform to the expected standard of x=(int)|y=(int)|w=(int)|h=(int)", ex);
        }
    }
     ...
     *
     * @param stepMajor Return the z-order incremented by a major step if true, a minor step if
     *                  false
     * @return float zOrder
     */
    public float incrementZOrder(final boolean stepMajor) {
    ...
    public void updateZOrder(final Element topMost) {
        ...
        for (final Element element : elements.array()) {
    ...
    public void setUseMultiTouch(boolean useMultiTouch) {
        ...
        application.getInputManager().setSimulateMouse(!useMultiTouch);
        application.getInputManager().setSimulateKeyboard(!useMultiTouch);
    }
    ...
    public void addSubScreen(SubScreen subscreen) {
        if (getSubScreenById(subscreen.getUID()) != null) {
            try {
               ...
               LOGGER.error("The SubScreen '" + subscreen.getUID() + "' (" + subscreen.getClass() + ") conflicts with a previously added subscreen in parent Screen.", ex);
                System.exit(0);
            }
        } else {
    ...
    private void setLastCollision() {
        if (Screen.isAndroid())
            ...
            click2d.set(mouseXY);
        tempV2.set(click2d);
        click3d.set(application.getCamera().getWorldCoordinates(tempV2, 0f));
        pickDir.set(application.getCamera().getWorldCoordinates(tempV2, 1f).subtractLocal(click3d).normalizeLocal());
        pickRay.setOrigin(click3d);
        pickRay.setDirection(pickDir);
        CollisionResults results = new CollisionResults();
        application.getViewPort().getScenes().get(0).collideWith(pickRay, results);
	...
	*/
    ...
    /**
     * Sets the current stored text to the internal clipboard.  This is probably going to vanish
     * quickly.
     *
    ...
    /**
     * Returns a pointer to the AnimManager.  The AnimManager is a time based queuing for
     * TemporalActions used with @Transformable ( See @AnimElement @QuadData )
     *
    ...
    public AnimLayer addAnimLayer(String UID) {
        if (getAnimLayerById(UID) != null) {
            try {
                ...
                LOGGER.error( "The child layer '" + UID + "' (Element) conflicts with a previously added child layer in parent Screen.", ex);
                System.exit(0);
            }
            return null;
        } else {
            ...
            if (!layer.getInitialized()) {
                final Vector2f orgPosition = layer.getOrgPosition();
                orgPosition.set(layer.getPosition());
    ...
    public Control cloneForSpatial(Spatial spatial) {
        Screen screen = new Screen(this.application, styleManager.getStyleMap());
        screen.idToElement.put(this.idToElement);
        return screen;
    }
    ...
    /**
     * Sets the overall opacity of all idToElement that have not been flagged as
    ...
    /**
     * Resets the tab focus element to null after calling the TabFocusListener's resetTabFocus
     * method.
     */
    @Override
    ...
    private Node getEventNode(float x, float y) {
        ...
        for (ViewPort vp : application.getRenderManager().getMainViews()) {
            Node root = (Node) vp.getScenes().get(0);

            if (!Screen.isAndroid())
                click2d.set(application.getInputManager().getCursorPosition());
            else
                click2d.set(touchXY);
            tempV2.set(click2d);
